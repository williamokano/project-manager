...
//    if (unsaveChanges) {
//        return "Atenção!\nExistem alterações que ainda não foram salvas!\nDeseja realmente continuar?\nAs alterações realizadas e não salvas serão perdidas.";
//    }
//}

/* Configuração tagsinput e autocomplete para usuário */
var configAutocomplete = {
    ...
    select: function (event, ui) {
        ...
        $(this).parent().parent().find(">input").tagsinput('add', { "DsNome": ui.item.label, "IdUsuario": ui.item.idUsuario });
        $(this).val("");
        return false;
    }
};
...
/* Fim da Configuração tagsinput e autocomplete para usuário */
...
function GeraObjPost() {
    ...
    $("li[data-lid]").each(function () {
        var tagsAtividade = $(this).find("#txtTags").tagsinput('items');
        if (tagsAtividade.length > 0 && tagsAtividade[0] instanceof Array)
            tagsAtividade = [];

        var usuariosAtividade = $(this).find("#txtUsuarios").tagsinput('items');
        if (usuariosAtividade.length > 0 && usuariosAtividade[0] instanceof Array)
            usuariosAtividade = []
        dados.push({
            ...
            QtTempoExecutado: $(this).find("#txtQtTempoExecutado").val(),
            VrCompletoPorcentagem: $(this).find("#txtVrCompletoPorcentagem").val(),
            DtInicioStr: $(this).find("#txtDtInicio").val(),
            DtTerminoStr: $(this).find("#txtDtTermino").val(),
            DsAtividade: $(this).find("#txtDescricaoAtividade").val(),
            Tags: tagsAtividade,
            Usuarios: usuariosAtividade
        });
    });
...
function Salvar() {
    ...
    if (qtErr > 0) {
        ...
        ExecutaFuncao(PegarEnderecoPagina() + "/Salvar", { atividades: objPost, idProjeto: IdProjeto }, function (response) {
...
function CriaLiAtividade(IdLocal) {
    ...
    var $txtQtTempoExecutado = $("<div/>").addClass("form-group")
                                        .append($("<label/>")
                                                    .attr("for", "txtQtTempoExecutado")
                                                    .html("Tempo Executado")
                                                    .addClass("control-label"))
                                        .append($("<input/>")
                                                    .attr("type", "text")
                                                    .attr("readonly", "true")
                                                    .addClass("form-control campo-tempo")
                                                    .attr("id", "txtQtTempoExecutado")
                                                    .val("000:00"))
                                        .append('<span class="help-block error-block">Este campo é obrigatório e requer um valor maior do que 0.</span>');
    $form.append($txtQtTempoExecutado);

    var $txtVrCompletoPorcentagem = $("<div/>").addClass("form-group")
                                        .append($("<label/>")
                                                    .attr("for", "txtVrCompletoPorcentagem")
                                                    .html("% completa")
                                                    .addClass("control-label"))
                                        .append($("<input/>")
                                                    .attr("type", "text")
                                                    .addClass("form-control campo-numero requer-validacao valida-numero valida-numero-range")
                                                    .attr("data-range", "0~100")
                                                    .data("range", "0~100")
                                                    .attr("id", "txtVrCompletoPorcentagem"))
                                        .append('<span class="help-block error-block">Este campo é obrigatório e requer um valor de 0 à 100.</span>');
    $form.append($txtVrCompletoPorcentagem);

    var $txtDtInicio = $("<div/>").addClass("form-group")
                                        .append($("<label/>")
                                                    .attr("for", "txtDtInicio")
                                                    .html("Dt. Início")
                                                    .addClass("control-label"))
                                        .append($("<input/>")
                                                    .attr("type", "text")
                                                    .addClass("form-control campo-data requer-validacao valida-data")
                                                    .attr("id", "txtDtInicio"))
                                        .append('<span class="help-block error-block">Este campo é obrigatório e requer uma data válida.</span>');
    $form.append($txtDtInicio);

    var $txtDtTermino = $("<div/>").addClass("form-group")
                                        .append($("<label/>")
                                                    .attr("for", "txtDtTermino")
                                                    .html("Dt. Término")
                                                    .addClass("control-label"))
                                        .append($("<input/>")
                                                    .attr("type", "text")
                                                    .addClass("form-control campo-data requer-validacao valida-data")
                                                    .attr("id", "txtDtTermino"))
                                        .append('<span class="help-block error-block">Este campo é obrigatório e requer uma data válida.</span>');
    $form.append($txtDtTermino);

    var $inputNome = $("<input/>")
                        .attr("type", "text")
                        .addClass("form-control nome-autocomplete")
                        .attr("id", "txtUsuarios");
    var $txtUsuarios = $("<div/>").addClass("form-group")
                                        .append($("<label/>")
                                                    .attr("for", "txtUsuarios")
                                                    .html("Usuários")
                                                    .addClass("control-label"))
                                        .append($inputNome);
    $form.append($txtUsuarios);
...
function AplicaPluginsJs($li) {
    ...
    $inputNome.tagsinput(tagsInputConf);
    $($inputNome.tagsinput('input')).autocomplete(configAutocomplete);
